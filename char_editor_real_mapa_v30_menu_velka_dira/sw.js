const CACHE_NAME='velka-dira-v57';
...
const urlOf=f=>'./'+String(f).replace(/^\.\//,'');
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    const cache=await caches.open(CACHE_NAME);
    for(const a of ASSETS){
      try{
        const req=new Request(urlOf(a),{cache:'reload'});
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('velka-dira-v')&&k!==CACHE_NAME).map(k=>caches.delete(k)));
